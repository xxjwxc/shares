...
#东财日线
def get_price_day_dc(code, end_date='', count=10, frequency='1d'):     #日线获取 
    ext='1' if code[:2] == 'sh' else '0'
    sampleCode = code[2:]
    tmStr = (datetime.datetime.now()+datetime.timedelta(days=-count)).strftime("%Y%m%d")   
    URL=f'http://push2his.eastmoney.com/api/qt/stock/kline/get?fields1=f1,f2,f3&fields2=f51,f52,f53,f54,f55,f56,f59&klt=101&fqt=1&secid={ext}.{sampleCode}&beg={tmStr}&end=20500101' 
    st= json.loads(requests.get(URL).content); stk=st['data']['klines']  
    buf = [item.split(',') for item in stk]
    buf[-1][-1] = buf[-1][-1] + '00'  
    df=pd.DataFrame(buf,columns=['time','open','close','high','low','volume','percent'])     
    df[['open','close','high','low','volume','percent']]=df[['open','close','high','low','volume','percent']].astype('float')   
    df.time=pd.to_datetime(df.time);    df.set_index(['time'], inplace=True);   df.index.name=''          #处理索引 
    df['daytp']="dc"
    return df
...
def get_price(code, end_date='',count=10, frequency='1d', fields=[]):        #对外暴露只有唯一函数，这样对用户才是最友好的  
    ...
    if  frequency in ['1d','1w','1M']:   #1d日线  1w周线  1M月线
        ...
        except: return get_price_day_dc(xcode,end_date=end_date,count=count,frequency=frequency)   #备用                    
...
if __name__ == '__main__':    
    df=get_price('sh688343',frequency='1d',count=10)      #支持'1d'日, '1w'周, '1M'月  
    print('上证指数日线行情\n',df)
