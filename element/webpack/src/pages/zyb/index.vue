<template>
  ...
  <div id="header">
    <div class="left">
      <div class="name"> 中意榜 </div>
      <div class="zde" style="background-color: rgba(255, 0, 0, 0);color:#0000ff">基准时间:{{ startTm }}</div>
      <div class="prise" style="background-color: rgba(255, 0, 0, 0);color:#000000">显示超跌:
        <el-switch v-model="showCd" @change="changeShowCd" />
      </div>
    </div>
    <div class="right">

      <el-tag class="block" :style="{ color: item.color }" v-for="item in tags" :key="item.doc" type="primary">
        {{ item.doc }}
      </el-tag>
    </div>
  </div>
  ...
  <div id="root">
    ...
    <div class="righ">
      ...
      <h2 style="font-size: 20px;color: #aa00ff;">行业中意榜</h2>
      ...
      <el-table @row-click="rowClick" :data="tsList1.list" style="width: 100%" height="400" :row-style="tableRowStyle">
        <el-table-column prop="name" label="股票名字" width="100">
          <template #default="{ row }">
            <span class="num-text">{{ row.ext }}</span>{{ row.name }}
          </template>
        </el-table-column>
        <el-table-column prop="sPrice" label="行业叠加" width="250" />
        <el-table-column prop="price" label="涨跌幅" />
      </el-table>
      <h2 style="font-size: 20px;color: #aa00ff;">个股中意榜</h2>
      <el-table @row-click="rowClick" :data="tsList2.list" style="width: 100%" height="400" :row-style="tableRowStyle">
        ...
        <el-table-column prop="sPrice" label="出现次数" width="150" />
        <el-table-column prop="price" label="涨跌幅" />
      </el-table>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    async loadData(options) {
      ...
      let group = await Shares.GetZybHyKline(this.btnActive1, this.showCd); // 获取分组信息
      if (group != null) {
        ...
        tooltip: {
          ...
          formatter: function (param) {
            ...
            return param.seriesName + '</br>' +
              '融资：' + value[0].toFixed(2) + '亿</br>' +
              '涨幅：' + value[1].toFixed(2) + '%</br>' +
              '成交量：' + value[2].toFixed(2) + '亿</br>' +
              '北向：' + value[3].toFixed(2) + '亿</br>'
          }
        },
        xAxis: {
          type: 'value',
      ...
      this.cjlchart.on("click", function (params) {
        ...
        if (that.clickIndex == params.seriesIndex) {
          ...
          if (timeDiff < 1000) {
            ...
            router.push("hydayliy?scode=" + code)
          }
        }
